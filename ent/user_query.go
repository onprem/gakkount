...
func (uq *UserQuery) QueryCourse() *CourseQuery {
	query := &CourseQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withCourse; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].course_users; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "course_users" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := uq.withDepartment; query != nil {
		...
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].department_users; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "department_users" returned %v`, n.ID)
			}
			for i := range nodes {
