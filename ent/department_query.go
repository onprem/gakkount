...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	...
	if query := dq.withUsers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.department_users
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "department_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "department_users" returned %v for node %v`, *fk, n.ID)
			}
